...
namespace Backend.Services
{
    public class CosmeticsServices
    {
        ...
        public async Task<List<CosmeticData>> GetFirst50BrCosmeticsAsync()
        {
            var client = _httpFactory.CreateClient("FortniteApi");
            var response = await client.GetFromJsonAsync<CosmeticsResponse>("cosmetics");

            if (response?.Data?.Br == null) return new List<CosmeticData>();

            return response.Data.Br.Take(50).ToList();
        }
